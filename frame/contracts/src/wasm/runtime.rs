...
fn read_sandbox_memory<E: Ext>(
	...
	charge_gas(ctx.gas_meter, ctx.schedule, &mut ctx.special_trap, RuntimeToken::ReadMemory(len))?;
...
fn read_sandbox_memory_into_buf<E: Ext>(
	...
	charge_gas(ctx.gas_meter, ctx.schedule, &mut ctx.special_trap, RuntimeToken::ReadMemory(buf.len() as u32))?;
...
fn write_sandbox_memory<T: Trait>(
	...
	charge_gas(gas_meter, schedule, special_trap, RuntimeToken::WriteMemory(buf.len() as u32))?;
